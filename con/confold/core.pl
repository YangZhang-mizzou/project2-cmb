#!/usr/bin/perl -w
# CONFOLD version 2.0, Badri Adhikari, 6/20/2017
# - Added the new soft-square energy term with rswitch parameter
# - Fixed two bugs: 'O atom not being included' and 'scalecoolsetup being used as scalehot'
# - Fixed sorting with exponential contact confidence values
...
my $cns_suite      = "/Users/zhangyang/Desktop/project2/cns_solve_1.3";
my $cns_executable = "$cns_suite/intel-x86_64bit-linux/bin/cns_solve";

# User inputs 
my ($dir_out, $file_fasta, $file_pair, $file_rr, $file_ss, $atomselect, $select_rr, $rrtype, $stage2, $rr_tbl, $mini, $drelaxation, $arelaxation);
...
# Options for the running the second stage
# '0' - don't run stage2
...
$stage2 = 1 if not defined $stage2;
...
load_ss_restraints("ssrestraints.log");
...
sub process_parameters{
	...
	confess "ERROR! It looks like the output directory is not empty! Please empty $dir_out !" if -d "$dir_out/input";
	...
	print2file("$dir_out/input/$id.fasta", ">$id");
	print2file("$dir_out/input/$id.fasta", "$seq");
	...
	if($file_ss){
		...
		if (length(seq_fasta($file_ss)) != length(seq_fasta($file_fasta))){
	...
	print "\n";
	print "\n".seq_fasta($file_fasta)."(input-sequence)";
...
sub contact_restraints{
	...
	if($stage eq "stage1"){
		...
		# Special case when tbl is direct input (used for debugging)
		if (defined $rr_tbl){
			print "\n\nTBL file $rr_tbl supplied ..\n";
			confess "ERROR!! $rr_tbl not found!" if not -f $rr_tbl;
			if (-f $rr_tbl){
				print "\n\nUsing input tbl $rr_tbl instead of input RR file..\n";
				system_cmd("rm ./$id.rr");
				system_cmd("rm contact.tbl");
				system_cmd("cp $rr_tbl ./contact.tbl");
				my $tbl_rr = $rr_tbl;
				$tbl_rr =~ s/contact.tbl/contact.rr/g;
				confess "Corresponding rr file $tbl_rr not found!" if not -f $tbl_rr;
				system_cmd("cp $tbl_rr ./$id.rr");
				my %confidence = rr2contacts_hash($tbl_rr, $min_seq_sep, 100000, "confidence");
				print2file("$id.rr", seq_rr($tbl_rr));
				foreach (keys %confidence){
					print2file("$id.rr", $_." 0 8 ".$confidence{$_}."\n");
				}
			}
		}
		return;
...
sub detect_hbonds_from_model{
	...
	foreach my $pairing (keys %pairs_subsets_info){
		...
		foreach my $subset (keys %subsets_info){
			...
			print2file("pairs_and_distances.log", (sprintf "%15s : %15s : %42s : cacadist = %.3f ", $pairing, $subset, $subsets_info{$subset}, $avg_ca_ca), "");
			if ($avg_ca_ca > $pthres){
				print2file("pairs_and_distances.log", " -> (ignoring because avg_ca_ca $avg_ca_ca > pthres $pthres )");
				next;
			}
			...
			print2file("pairs_and_distances.log", (sprintf "-> hbonddist = %.3f ", $avg_hb_dist), "");
			if ($avg_hb_dist > $pthres_hbond){
				print2file("pairs_and_distances.log", " -> (ignoring because avg_hb_dist $avg_hb_dist > pthres_hbond $pthres_hbond )");
				next;
			}
			# Step (3) average them.
			my $farness = ($avg_ca_ca + $avg_hb_dist)/2;
			print2file("pairs_and_distances.log", " -> mean = $farness", "");
			if (not defined $subsets_dist{$subset}){
				print2file("pairs_and_distances.log", " -> adding this", "");
				$subsets_dist{$subset} = $farness;
			}
			if ($subsets_dist{$subset} > $farness){
				print2file("pairs_and_distances.log", " -> replacing with this one because existing distance ".$subsets_dist{$subset}." > farness $farness", "");
				$subsets_dist{$subset} = $farness;
			}
	...
	foreach my $pairing (sort {$pairs_rank{$a} <=> $pairs_rank{$b}} keys %pairs_rank){
		...
		foreach my $best_subset (sort {$subsets_dist{$a} <=> $subsets_dist{$b}} keys %subsets_dist){
			...
			if($T[4] ne $direction_of_top1){
				print2file("pairs_and_distances.log", "\t\t$best_subset -> direction is different.. ignoring this and rest!");
				last;
			}
			my $addable = 0;
			$addable = is_addable_to_hbond_list($best_subset, %hbonds);
			if ($addable){
				print2file("pairs_and_distances.log", "\t\t$best_subset -> looks good, adding this!");
				...
				print2file("pairs_and_distances.log", "\t\t$best_subset -> could not add because of hbond issues!");
			}
		}
	}
	print2file("pairs_and_distances.log", "");
...
sub count_satisfied_tbl_rows{
	...
	if ($type eq "dihedral"){
		...
		foreach (sort keys %tbl_hash){
			...
			$log_rows{(sprintf "%3s\t%.2f\t%.2f # $_", $viol_flag, $deviation, $pdb_distance)} = $deviation;
			$total++;
		}
	}
...
sub sum_noe_dev{
	my $file_pdb = shift;
	my $file_tbl = shift;
	my $flag_weighted = shift;
	$flag_weighted = 0 if not defined $flag_weighted;
	confess "ERROR! file pdb $file_pdb does not exist!" if not -f $file_pdb;
	confess "ERROR! file tbl $file_tbl does not exist!" if not -f $file_tbl;
	my $sum_dev = 0.0;
	my %tbl_hash = ssnoe_tbl_min_pdb_dist($file_tbl, $file_pdb);
	if ($flag_weighted){
		my %tbl_hash_rest_weight = ssnoe_tbl_wt_hash($file_tbl);
		foreach (sort keys %tbl_hash){
			my $viol_flag = 1;
			...
			if ($pdb_distance > ( $D[0] + $D[2] + 0.2) ){
				...
				$deviation *= $tbl_hash_rest_weight{$_};
				$sum_dev += $deviation;
			}
			if ($pdb_distance < ( $D[0] - $D[1] - 0.2) ){
				$deviation = ( $D[0] - $D[1] ) - $pdb_distance;
			...
			# print $_." actual-dev:".($deviation/$tbl_hash_rest_weight{$_})." eff-deviation:$deviation\n";
		}
...
sub rr_remove_unsatisfied_top_model{
	...
	my %confidence = rr2contacts_hash($file_rr, $min_seq_sep, 100000, "confidence");
	my %rr_thres = rr2contacts_hash($file_rr, $min_seq_sep);
...
sub assess_dgsa{
	...
	foreach my $pdb (sort {$energy_noe{$a} <=> $energy_noe{$b}} keys %energy_noe){
		...
		$s1 = sum_noe_dev($pdb, "contact.tbl", 1) if defined $tbl_list{"contact.tbl"};
...
sub print_usage{
	my $param_info = <<EOF;
--------------------------------------------------------------------------------
CONFOLD v2.0 - build protein 3D models using contacts and secondary structures
--------------------------------------------------------------------------------
